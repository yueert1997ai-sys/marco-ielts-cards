...
“今日 50”训练模式会按手机本地日期，从完整 903 词中抽取当天固定的 50 个词。系统优先安排尚未训练的词，严格按 S → A → B 推进，同一优先级内部随机；完成一轮 903 词后自动开始新一轮。只有“认识”或“不会”会计入完成，跳过的词会在当天再次出现。第二天自动生成新题单，首页保留近 7 天完成日历与连续完成天数。
...
“不会的”、当前模式、S/A/B 筛选和每日训练记录保存在浏览器 `localStorage`。普通随机模式每次重新打开或刷新网页都会重新洗牌；每日训练会保留当天已完成的词，并重新打乱尚未完成的题目。清除浏览器网站数据或换设备后，记录不会自动同步。
